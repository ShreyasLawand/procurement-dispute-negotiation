...
const BatchMetricsSchema = z.object({
  // Renamed from agreement_rate (Aug 2026) — see the same note in lib/schema.ts.
  // sync-data.mjs normalizes the legacy key before writing manifest.json, so this
  // schema (unlike schema.ts's) does not need to accept the old name.
...
const ManifestBatchEntrySchema = z.object({
  ...
  // Added Aug 2026. Defaulted rather than required: a manifest generated by an
  // older sync-data (or from batches predating these fields) must still validate,
  // otherwise the whole Analytics page fails closed over a provenance field.
  complete: z.boolean().nullable().default(null),
  courtPromptVersion: z.string().nullable().default(null),
  structuralComplianceRate: z.number().nullable().default(null),
});
