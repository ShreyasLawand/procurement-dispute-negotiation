...
(Figure~\ref{fig:lifecycle}). Nearly all the leverage sits in the first fortnight,
before anyone has instructed a solicitor, and so does nearly all the information
asymmetry: the authority knows exactly what its evaluation panel did, while the
losing bidder knows only what the assessment summary chose to tell it. Once the
automatic suspension kicks in and the case reaches the TCC, both sides are locked
into a process that, across this project's corpus, took four months to two years
to resolve.
...
The technical territory is multi-agent LLM simulation: several instances of a
language model, each assigned a role, a set of interests and a private fallback
...
\cite{wei2022chain}, the prompt became a programming surface. From there it is a
short step to running several copies of one model under different role prompts
and getting something that looks like a division of labour. That is what CAMEL
\cite{li2023camel} does, pairing an assistant and a user agent under an
inception-prompting scheme, and what Generative Agents \cite{park2023generative}
extended by giving twenty-five simulated townspeople memory, reflection and
planning. AutoGen \cite{wu2023autogen} and MetaGPT \cite{hong2024metagpt} pointed the same
idea at engineering workflows, and MetaGPT's authors argue that what stops
multi-agent conversation collapsing into noise is encoding standard operating
procedures into the prompts. That matters here: the Court agent's verification
protocol (\S\ref{sec:court-design}) is exactly such a procedure, closer to ReAct's
interleaving of reasoning with a defined action space \cite{yao2023react} than to
open-ended chat, and Chapter~\ref{ch:evaluation} shows what happens once case
shapes stop matching what it was written for. Orchestration runs on LangGraph
\cite{langgraph}, which represents the interaction as an explicit state graph
rather than free-form conversation, a choice defended in
\S\ref{sec:orchestration}.
...
That constraint is not a modelling convenience but a feature of the legal regime,
and it is why this system's five-outcome vocabulary is a closed statutory set
rather than a bargaining range. The remedies available to an aggrieved bidder ---
set-aside before contract, a declaration of ineffectiveness after it, and damages
--- descend from the EU Remedies Directive as amended in 2007
\cite{remediesdirective} and are carried into the Procurement Act 2023; Arrowsmith
\cite{arrowsmith2014law} treats them as the fixed menu a challenge must be argued
into. A negotiation simulator for this domain therefore cannot invent a settlement
shape, which is a stronger design constraint than most negotiation benchmarks
impose. It also frames what a simulator can usefully contribute: Genn
\cite{genn2009judging} argues that civil disputes overwhelmingly settle rather than
being adjudicated, and that the value of an early, cheap signal lies in informing
that settlement rather than in predicting a judgment nobody will seek. Fusion21's
Doc 4 makes the same observation about procurement specifically.
...
These break down into six objectives, which give Chapter~\ref{ch:methodology} its
structure and are assessed against the evidence in Table~\ref{tab:objectives}.

\begin{description}
  \item[O1] Specify three negotiating agents plus a non-negotiating summariser
    against Fusion21's own interest taxonomies.
  \item[O2] Build a deterministic, inspectable orchestration protocol with
    schema-validated output.
  \item[O3] Design a court agent that verifies conditionally rather than by
    default.
  \item[O4] Build an ingestion path from real documents into structured
    scenarios.
  \item[O5] Assemble a verified corpus of real UK judgments and evaluate against
    it with explicit baselines and ablations.
  \item[O6] Deliver something Fusion21 can use, which turned out to mean a
    pre-award risk screen rather than the simulator.
\end{description}
...
\begin{enumerate}
 ...
 (\S\ref{sec:court-design}): exact arithmetic only when both a formula and every
 input it needs are present, and invented values refused even when hedged as
 illustrative. Four prompt revisions are documented, each answering a specific
 measured failure.
 \item \textbf{An evaluation methodology for a domain with almost no ground
 truth} (Chapter~\ref{ch:evaluation}): agreement with real dispositions, three
 fabrication screens, structural-compliance instrumentation, a controlled prompt
 ablation with significance testing, and three baselines.
 \item \textbf{An outcome-leakage audit} (\S\ref{sec:leakage}) showing that six of
 fourteen merits scenarios state the real disposition in the text the agents
 reason over, and re-basing every accuracy claim on the subset that does not.
 \item \textbf{A negative result bounding the architecture claim}
 (\S\ref{sec:baselines}): a naive zero-shot prompt matches the full pipeline on
 directional accuracy, while removing the court agent deadlocks every run.
 Whatever the architecture earns, it earns on resolution mechanics, not accuracy.
 \item \textbf{Three distinct failure modes} evidenced on named cases
 (\S\ref{sec:fabrication}): numeric fabrication, narrative-premise fabrication and
 legal-judgment divergence, with the finding that hardening the adjudicator
 against the first does nothing about the second.
 \item \textbf{A verified corpus of 23 real UK procurement judgments}, rendered as
 structured scenarios with each disposition checked individually.
 \item \textbf{A pre-award challenge risk screen} (\S\ref{sec:risk-screen}), traced
 rule by rule to Fusion21's own practical-considerations tables and the only part
 of this system with an observable ground truth in principle.
\end{enumerate}

\section{Report structure}

Chapter~\ref{ch:methodology} describes the system and defends its design against
the alternatives rejected along the way; Chapter~\ref{ch:evaluation} presents the
evaluation, including the results that do not flatter the artefact; and
Chapter~\ref{ch:conclusion} returns to the objectives and sets out what should be
built next.