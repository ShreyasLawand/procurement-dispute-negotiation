...
for what the agents can be trusted to do, because a persuasive role performance is
not evidence the reasoning is sound. An early version used interest lists I had
written myself, which made the agents believable but gave nobody a way to
challenge them. Those were replaced with the taxonomies in Fusion21's Doc 1, so
every agent's stated interests trace to a named category in a document the client
wrote rather than to my guess at what a procurement officer cares about. Doc 1
sets out seven categories for the contracting authority, six for the supplier and
six for the judiciary; Table~\ref{tab:agent-config} summarises the result.
...
better bidder happened to win \cite{arrowsmith2014law}. Using a model to score
another model's output is now a common evaluation pattern
\cite{zheng2023judging}, and the same caution applies here as there, since the
...
What extraction deliberately does \emph{not} do is remove anything. There is no
pass that strips the eventual court result out of a source document before the
agents negotiate over it, so where a judgment states its own disposition that
sentence can survive into the scenario the agents receive.
Section~\ref{sec:leakage} measures how often that happens and reports every
accuracy figure against the subset where it does not.
