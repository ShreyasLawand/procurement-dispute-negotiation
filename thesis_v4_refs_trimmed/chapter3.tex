...
it. The real judge rejected exactly that, holding the no-caveats rule fair and
commonly used. Nothing was invented and the reasoning engages the real facts
throughout; the model reached a different legal conclusion on a genuinely arguable
point. That is a category of error worth separating from fabrication, and
\S\ref{sec:fabrication} does so.
...
and 7 of 8 clears it by a real margin. The fix is a pass stripping disposition
sentences at ingestion, straightforward for this corpus but not for arbitrary
uploads where a user may legitimately supply a judgment. The leak was measured
rather than removed, and every accuracy claim rests on the subset where it does
not apply.
...
legislation.gov.uk rather than assumed. Figure~\ref{fig:citations} shows how the 702 divide.
...
in round one to 0.470 later: it never concedes on opening, and concedes roughly
half the time once the exchange is underway. The bidder concedes far less, 0.000
to 0.075, matching what a bidder concession amounts to: dropping the challenge
entirely, a much larger move than offering extra transparency
(Figure~\ref{fig:concession}).
