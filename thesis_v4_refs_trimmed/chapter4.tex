...
O3 & Conditionally verifying court agent & \S\ref{sec:court-design};
 0 of 9 flagged instances Court-originated & Met, with a
...
on Bromcom the Court invented an illustrative dataset on a case it had
misclassified as numeric, which the screen missed on both its axes
(\S\ref{sec:fabrication}), and the arithmetic defect on the client's own scoring
table resisted three prompt revisions. The discipline constrains what the
adjudicator computes, not what premise it computes on --- and the screen bounds
what can be claimed about either.
