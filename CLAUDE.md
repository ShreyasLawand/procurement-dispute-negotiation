...
**Finding: 0/190 confirmed genuine Court fabrications after manual review of all 6 automated flags — but the flags themselves are not boring.** One is the Court correctly deriving a simple difference (79−65=14) from two grounded numbers — a false positive of this string-presence detector by design, not a bug. The other five are the Court explicitly attributing a number to a party's claim and declining to treat it as verified ("this is not a numeric value that can be independently verified" — an exact quote) — textbook-correct Step 2B behaviour. But three of those five are on Alstom, whose real record specifies a binary pass/fail threshold with no percentage scores anywhere — so the CA/Bidder agents are inventing quantitative specificity ("80%", "a minimum score of 85%... achieved 92%", "4 out of 10") the real case doesn't have. Not Court fabrication (the Court's caution is exactly right), but the same failure class as the Faraday finding: a fabricated premise from the negotiating agents that nothing currently checks against the scenario record.
...
**Findings over the full corpus:** 0 genuine retractions found (169 logs). CA concession rate jumps from 0.012 (round 1) to 0.571 (round 2+) — essentially never concedes on opening, concedes over half the time once negotiation is underway. Bidder concedes far less often even in later rounds (0.0 → 0.104) — consistent with a Bidder "concession" typically meaning dropping the challenge, a much bigger move than a CA offering additional transparency. Mean consecutive-message similarity is low (0.12 on a 0–1 scale), consistent with `is_repetitive()` (`negotiation_helpers.py`) successfully suppressing near-duplicate messages in production.
