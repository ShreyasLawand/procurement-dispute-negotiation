...
**Known cost of the deletion — the Court agent's Step 2A branch is now untested.** None of the four real judgments publishes a scoring formula with sub-scores, so nothing currently exercises exact-arithmetic verification. The V2→V3 finding can only be half-demonstrated: "does not fabricate on qualitative" is still testable, "computes exactly on numeric" is not. To restore it, add a real judgment that reproduces the authority's scoring arithmetic (they exist — scoring-error challenges are common) rather than reinstating a synthetic fixture.
