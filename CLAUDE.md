...
| Parkingeye v Velindre | 100% (8/8) | 100% (8/8) |

**V4 resolves at 100% on every case; V3 also reaches 100% on 2 of 5 (Woods, Parkingeye) but is lower on the other 3 (50-87.5% on Alstom, Faraday, Lancashire).** Re-running Parkingeye at n=8 (15 Aug 2026, replacing an n=4 legacy batch that pre-dated the `resolution_rate` rename) changed the read: this is not "V3 always lags," it's "V4 is uniformly at the ceiling, V3 varies by case." The precedence note appears to hold in the sense that V4 never invents a remedy inconsistent with the finding, but V4 never landing below 100% on any case is exactly the kind of settlement-encouraging bias Doc 1's category 5 predicts, and is worth naming as a finding rather than treating as simply "V4 is better." A prompt that resolves everything is not automatically the more *correct* one — this needs a qualitative read of a sample of V4 transcripts (does it fabricate, concede prematurely, or genuinely reach a soundly-reasoned resolution?) before citing "V4 improves resolution rate" as an unqualified positive in the dissertation or the board deck.
